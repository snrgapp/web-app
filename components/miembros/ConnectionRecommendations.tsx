...
export function ConnectionRecommendations() {
  ...
  async function handleInviteCafe(id: string) {
    setInvitingId(id)
    try {
      const res = await fetch('/api/miembros/invite-cafe', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ connectedMemberId: id }),
      })
      if (res.ok) {
                  ...
                  <tr key={r.id} className="border-b border-zinc-100">
                    ...
                    <td className="py-3">
                      <Button
                        ...
                        onClick={() => handleInviteCafe(r.id)}
                        disabled={invitingId === r.id}
                      >
                        <Coffee className="w-4 h-4 mr-1" />
                        {invitingId === r.id ? 'Enviando...' : 'Invitar un café'}
                      </Button>
                    </td>
                  </tr>
                ))}
