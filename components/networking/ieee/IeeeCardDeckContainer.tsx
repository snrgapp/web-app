...
import { IeeeFeedbackModal } from '@/components/networking/ieee/IeeeFeedbackModal'
...
export default function IeeeCardDeckContainer({ questions, ronda }: Props) {
  ...
  const handleFinalizar = () => {
    if (ronda === 1) {
      router.push('/networking/ieee?momento=tarde')
      return
    }
  ...
  const handleFeedbackComplete = () => {
    ...
    router.push('/networking/ieee/verify')
  }

  if (!submissionId) {
    return (
      <div className="flex min-h-dvh items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-white/20 border-t-[#00629B]" />
      </div>
    )
  }
  ...
  const accentClass = pulse ? 'text-[#00629B]' : 'text-white'
    ...
    >
      ...
      <header className="relative z-30 flex w-full items-center justify-between px-4 py-4 sm:px-6">
        ...
        <button
          ...
          className="rounded-full border border-white/18 bg-[#252525] px-5 py-2 text-xs font-medium uppercase tracking-wider text-white shadow-[4px_4px_0_#00629B] transition hover:border-white/28"
      ...
      <div className="relative z-[1] flex min-h-0 flex-1 flex-col px-4 pb-20 sm:px-6 sm:pb-28">
        <div className="relative z-10 flex min-h-0 flex-1 items-center justify-center px-2 py-2 sm:px-6">
          {countdown !== null ? (
            ...
            <motion.div
              ...
              className="relative"
            >
              <QuestionCard content={selectedQuestion.content} category={selectedQuestion.category} variant="ieee" />
            </motion.div>
          ) : null}
        </div>
          ...
          <div className="mx-auto mt-8 flex w-full max-w-xl flex-col items-center px-4 sm:-mt-10 sm:mt-0">
            ...
            >
              <svg
                ...
                stroke="currentColor"
                strokeWidth="2"
                className="shrink-0 text-[#00629B]"
              >
            ...
            <div className="flex items-start justify-center gap-6 sm:gap-10">
              ...
              <div className="flex flex-col items-center pt-2">
                <motion.button
                  type="button"
                  onClick={handleGirar}
                  className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-full border-2 border-[#00629B] bg-[#141414] text-xs font-medium uppercase tracking-wider text-white shadow-[4px_4px_0_rgba(0,98,155,0.4)]"
                  style={{ fontFamily: 'var(--font-geist-sans), sans-serif' }}
        ...
        <TimeUpNotification isOpen={showTimeUpNotification} onClose={() => setShowTimeUpNotification(false)} ieee />
      </div>
    </motion.div>
  )
}
