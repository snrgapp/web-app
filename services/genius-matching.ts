/**
 * Matching Genius FEST sin IA: mismas reglas que hackathon pero diversidad por `identidad`.
 * Aristas en match_genius (parejas dentro de cada grupo).
 */
...
const DEFAULT_RAZON =
  'Grupo Genius FEST: perfiles mixtos (idealmente hasta 2 de la misma identidad).'
...
function buildRound1Groups(profiles: GeniusProfile[], messages: string[]): string[][] {
  let pool = shuffle([...profiles])
  const groups: string[][] = []
  let relajo = false

  while (pool.length > 0) {
    const group: GeniusProfile[] = []
    const counts = new Map<string, number>()
    ...
    while (group.length < MAX_GROUP && pool.length > 0) {
      ...
      for (
        ...
        for (let i = 0; i < pool.length; i++) {
          const p = pool[i]!
          if ((counts.get(p.identidad) ?? 0) < ceiling) {
            group.push(p)
            pool.splice(i, 1)
            counts.set(p.identidad, (counts.get(p.identidad) ?? 0) + 1)
            placed = true
            break
          }
        }
  ...
  if (relajo) {
    messages.push(
      'Aviso: en algunos grupos hubo más de 2 de la misma identidad por falta de diversidad.'
    )
  }
  return groups
}
...
async function insertMatchChunks(
  ...
  for (let i = 0; i < rows.length; i += INSERT_CHUNK) {
    ...
    const { error } = await supabase.from('match_genius').insert(chunk as never)
    if (error) return { error: new Error(error.message) }
  }
  return { error: null }
}

export type RecomputeGeniusResult = {
  ok: boolean
  profileCount: number
...
export async function recomputeGeniusMatches(): Promise<RecomputeGeniusResult> {
  const messages: string[] = []

  const fail = (extra?: Partial<Omit<RecomputeGeniusResult, 'ok' | 'messages'>>) => ({
    ok: false as const,
  ...
  const { data: rows, error } = await supabase
    ...
    .select('id, nombre_completo, identidad, created_at')
    .order('created_at', { ascending: true })

  if (error) {
    console.error('recomputeGeniusMatches fetch:', error)
    messages.push(`Error leyendo inscritos: ${error.message}`)
    return fail({})
  }

  const profiles = (rows ?? []) as GeniusProfile[]
  const ids = profiles.map((p) => p.id)

  const sentinelUuid = '00000000-0000-0000-0000-000000000000'
  const { error: delErr } = await supabase.from('match_genius').delete().neq('id', sentinelUuid)
  if (delErr) {
    messages.push(`No se pudo vaciar match_genius: ${delErr.message}`)
  ...
  messages.push(
    'Matching Genius FEST: grupos de hasta 5 con hasta 2 de la misma identidad cuando hay diversidad; ronda 2 sin repetir parejas de ronda 1.'
  )
  ...
  if (insErr) {
    console.error('insert match_genius:', insErr)
